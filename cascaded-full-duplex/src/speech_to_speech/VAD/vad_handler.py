...
class VADHandler(BaseHandler[VADIn, VADOut]):
    ...
    def setup(
        ...
        streaming_audio_chunks: bool = False,
        ...
        self.streaming_audio_chunks = streaming_audio_chunks
        ...
        self._streaming_released_samples: int = 0
    ...
    def _start_new_turn(self) -> tuple[str, int]:
        ...
        self._streaming_released_samples = 0
    ...
    def _drop_superseded_vad_audio(self, latest: VADAudio) -> int:
        # Cumulative progressive audio can be superseded safely. Streaming
        # ASR chunks cannot: dropping one would create a hole in its cache.
        if getattr(self, "streaming_audio_chunks", False):
            return 0
        ...
        if self.enable_realtime_transcription and hasattr(self.iterator, "buffer") and len(self.iterator.buffer) > 0:
            ...
            if (current_time - self.last_process_time) >= progressive_pause:
                ...
                if active_speech_duration_ms >= self._active_speech_min_ms(start_ms):
                    ...
                    if getattr(self, "streaming_audio_chunks", False):
                        # Streaming ASR receives only new samples. Its cache
                        # carries the context from earlier chunks.
                        new_array = array[self._streaming_released_samples :]
                        self._streaming_released_samples = len(array)
                    else:
                        new_array = self._combined_turn_audio(array)
                    if len(new_array) == 0:
                        new_array = array
                    yield VADAudio(
                        audio=new_array,
        ...
        if vad_output is not None:
            ...
            if active_speech_duration_ms < min_active_ms or duration_exceeds_limit:
                ...
                final_audio = (
                    ...
                    else output_array[self._streaming_released_samples :]
                )
                yield VADAudio(
                    audio=final_audio,
                    runtime_config=runtime_config,
