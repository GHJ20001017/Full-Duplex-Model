...
async def _dispatch_client_event(
    ...
    *,
    ...
    # This private client-to-server event is intentionally outside the public
    # OpenAI Realtime schema: it requests a fixed TTS acknowledgement only.
    if raw.get("type") == "wake_word.detected":
        result = service.handle_wake_word(session_id, raw.get("text", ""))
        if result:
            await send_correlated([result])
            if result.type == "response.created":
                service.response.mark_response_created_sent(
                    session_id,
                    service._state(session_id).current_response_key,
                )
        return
