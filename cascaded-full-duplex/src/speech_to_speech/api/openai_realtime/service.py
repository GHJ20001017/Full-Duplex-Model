...
from speech_to_speech.pipeline.messages import EndOfResponse, GenerateResponseRequest, TTSInput
...
class ConnState(BaseModel):
    ...
    # A VAD speech start is only a possible barge-in. The turn controller
    # confirms it from STT before the active response is cancelled.
    pending_barge_in: bool = False
    pending_barge_in_started_at: float | None = None
...
class RealtimeService:
    ...
    def __init__(
        ...
        tts_input_queue: Queue[Any] | None = None,
        ...
        self._pipeline_dispatch: dict[type[PipelineEvent], Callable[..., list[ServerEvent]]] = {
            ...
            PartialTranscriptionEvent: self._on_partial_transcription,
    ...
    def handle_wake_word(self, conn_id: str, text: str) -> ServerEvent | None:
        """Queue a fixed wake acknowledgement directly into the TTS stage."""
        if not isinstance(text, str) or not text.strip():
            return self.make_error("Wake acknowledgement text must not be empty", "invalid_wake_word_event")
        st = self._state(conn_id)
        tts_queue = self.tts_input_queue
        if tts_queue is None:
            return self.make_error("TTS queue is not available", "wake_word_tts_unavailable")
        if st.in_response or st.response_pending:
            logger.debug("Ignoring wake acknowledgement while response is active")
            return None

        response = RealtimeResponseCreateParams(output_modalities=["audio"])
        response_key = _generate_id("wake")
        st.in_response = True
        st.current_response_id = _generate_id("resp")
        st.current_response_key = response_key
        st.current_response_params = response
        st.response_created_pending_key = response_key
        ...
        tts_queue.put(EndOfResponse(response_key=response_key))
        logger.info("Wake acknowledgement queued directly to TTS: %s", text.strip())
    ...
    def _clear_barge_in_candidate(self, conn_id: str) -> None:
        st = self._state(conn_id)
        st.pending_barge_in = False
        st.pending_barge_in_started_at = None
    ...
    def _evaluate_barge_in(
        ...
        *,
        ...
        if st.pending_barge_in_turn_id is not None and (
            turn_id != st.pending_barge_in_turn_id
            or turn_revision != st.pending_barge_in_turn_revision
        ):
            return None
        started_at = st.pending_barge_in_started_at or monotonic()
        duration_ms = max(0.0, (monotonic() - started_at) * 1000.0)
        decision = self.turn_controller.decide(
            transcript,
            speech_duration_ms=duration_ms,
            assistant_is_speaking=(
                st.in_response and st.current_response_key == st.pending_barge_in_response_key
            ),
            final=final,
        )
        logger.debug(
            "Turn controller decision=%s duration_ms=%.0f transcript=%r",
            decision.value,
            duration_ms,
            transcript,
        )
    ...
    def _cancel_confirmed_barge_in(self, conn_id: str) -> list[ServerEvent]:
        st = self._state(conn_id)
        events = self.response.finish_response(conn_id, status="cancelled", reason="turn_detected")
        self.response.discard_tool_followup_prefetch(conn_id)
        st.generation_done_tool_calls.clear()
        st.completed_tool_response_keys.clear()
        self._clear_barge_in_candidate(conn_id)
        logger.info("Turn controller confirmed barge-in; cancelled active response")
        return events
    ...
    def _confirm_barge_in(
        ...
        *,
        ...
        if decision is not TurnDecision.CANCEL:
            return []
        return self._cancel_confirmed_barge_in(conn_id)

    def _on_partial_transcription(self, conn_id: str, event: PartialTranscriptionEvent) -> list[ServerEvent]:
        """Confirm a possible interruption before publishing the STT delta."""
        events = self._confirm_barge_in(
            conn_id,
            event.delta,
    ...
    def _on_transcription_completed(self, conn_id: str, event: TranscriptionCompletedEvent) -> list[ServerEvent]:
        ...
        interruption_events = (
            self._cancel_confirmed_barge_in(conn_id)
            if decision is TurnDecision.CANCEL
            else []
        )
        suppress_response = decision is TurnDecision.BACKCHANNEL
        self._clear_barge_in_candidate(conn_id)
        ...
        if transcript and not suppress_response:
        ...
        if queue and transcript and not suppress_response:
