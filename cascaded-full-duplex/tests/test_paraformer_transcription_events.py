...
class _FakeStreamingParaformerModel:
    def __init__(self):
        self.calls = []
    ...
    def generate(self, **kwargs):
        ...
        return [{"text": "明天杭州" if not kwargs["is_final"] else "天气"}]
...
def test_streaming_paraformer_reuses_cache_and_flushes_final(monkeypatch):
    monkeypatch.setattr(paraformer_handler.console, "print", lambda *args, **kwargs: None)
    monkeypatch.setattr(paraformer_handler.torch.mps, "empty_cache", lambda: None)
    model = _FakeStreamingParaformerModel()
    handler = object.__new__(ParaformerSTTHandler)
    handler.model = model
    handler.language = "zh"
    handler.device = "cpu"
    ...
    assert isinstance(result[0], Transcription)
    assert result[0].text == "明天杭州天气"
    assert ("t", 0) not in handler._streaming_sessions
