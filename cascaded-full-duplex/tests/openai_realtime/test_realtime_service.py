...
class TestDispatchPipelineEvent:
    ...
    def test_turn_controller_cancels_after_meaningful_transcription(self, service, conn_id):
        ...
        assert service._state(conn_id).pending_barge_in is True

        events = service.dispatch_pipeline_event(
            conn_id,
            PartialTranscriptionEvent(delta="等一下"),
        )
        ...
        assert done.response.status == "cancelled"
        assert done.response.status_details.reason == "turn_detected"
        assert service._state(conn_id).pending_barge_in is False

    def test_turn_controller_keeps_response_for_backchannel(self, service, conn_id):
        service.response._ensure_response(conn_id)
        service.dispatch_pipeline_event(conn_id, SpeechStartedEvent())

        events = service.dispatch_pipeline_event(
            conn_id,
            PartialTranscriptionEvent(delta="嗯"),
        )

        assert not any(isinstance(e, ResponseDoneEvent) for e in events)
    ...
    def test_backchannel_final_does_not_enqueue_new_response(self, service, conn_id, text_prompt_queue):
        ...
        events = service.dispatch_pipeline_event(
            conn_id,
            TranscriptionCompletedEvent(transcript="嗯"),
        )

        assert any(isinstance(e, ConversationItemInputAudioTranscriptionCompletedEvent) for e in events)
        assert text_prompt_queue.empty()
        assert service._state(conn_id).in_response is True
