...
def test_audio_client_discards_late_audio_from_cancelled_response(capsys):
    ...
    handle_server_event(
        SimpleNamespace(type="response.created", response=SimpleNamespace(id="old")),
        playback=playback,
        renderer=renderer,
        print_json=False,
    )
    handle_server_event(
        SimpleNamespace(type="response.output_audio.delta", response_id="old", delta="AQI="),
        playback=playback,
        renderer=renderer,
        print_json=False,
    )
    ...
    handle_server_event(
        SimpleNamespace(type="response.output_audio.delta", response_id="old", delta="AwQ="),
        playback=playback,
        renderer=renderer,
        print_json=False,
    )
...
def test_audio_client_old_cancelled_response_cannot_clear_new_response(capsys):
    playback = PlaybackBuffer(16000)
    renderer = _FriendlyEventRenderer()

    for response_id in ("old", "new"):
        handle_server_event(
        ...
        handle_server_event(
            SimpleNamespace(type="response.output_audio.delta", response_id=response_id, delta="AQI="),
            playback=playback,
            renderer=renderer,
            print_json=False,
        )
    ...
    handle_server_event(
        SimpleNamespace(type="response.done", response=SimpleNamespace(id="old", status="cancelled")),
        playback=playback,
        renderer=renderer,
        print_json=False,
    )
